...
func TestHelmfilePlugin(t *testing.T) {
	...
	for _, b := range plugin.Spec.Binaries {
		...
		case "arm64":
			if b.Goos == "Linux" {
				foundArm = true
				assert.Equal(t, "https://github.com/helmfile/helmfile/releases/download/v"+plugins.HelmfileVersion+"/helmfile_linux_arm64", b.URL, "URL for linux arm binary")
				t.Logf("found linux binary URL %s", b.URL)
			}
		...
		case "amd64":
			...
			case "Darwin":
				foundMac = true
				assert.Equal(t, "https://github.com/helmfile/helmfile/releases/download/v"+plugins.HelmfileVersion+"/helmfile_darwin_amd64", b.URL, "URL for linux binary")
				t.Logf("found mac binary URL %s", b.URL)
			case "Linux":
				foundLinux = true
				assert.Equal(t, "https://github.com/helmfile/helmfile/releases/download/v"+plugins.HelmfileVersion+"/helmfile_linux_amd64", b.URL, "URL for linux binary")
				t.Logf("found linux binary URL %s", b.URL)
			case "Windows":
				foundWindows = true
				assert.Equal(t, "https://github.com/helmfile/helmfile/releases/download/v"+plugins.HelmfileVersion+"/helmfile_windows_amd64.exe", b.URL, "URL for windows binary")
				t.Logf("found windows binary URL %s", b.URL)
			}
		}
	}
