...
func TestHelmfilePlugin(t *testing.T) {
	...
	for _, b := range plugin.Spec.Binaries {
		...
		case "arm64":
			...
			case "Linux":
				foundArm = true
				assert.Equal(t, "https://github.com/roboll/helmfile/releases/download/v"+plugins.HelmfileVersion+"/helmfile_"+plugins.HelmfileVersion+"_Linux_arm64.tar.gz", b.URL, "URL for arm linux binary")
				t.Logf("found linux binary URL %s", b.URL)
			}
		...
		case "amd64":
			...
			case "Darwin":
				foundMac = true
				assert.Equal(t, "https://github.com/roboll/helmfile/releases/download/v"+plugins.HelmfileVersion+"/helmfile_"+plugins.HelmfileVersion+"_Darwin_amd64.tar.gz", b.URL, "URL for linux binary")
				t.Logf("found mac binary URL %s", b.URL)
			case "Linux":
				foundLinux = true
				assert.Equal(t, "https://github.com/roboll/helmfile/releases/download/v"+plugins.HelmfileVersion+"/helmfile_"+plugins.HelmfileVersion+"_Linux_amd64.tar.gz", b.URL, "URL for linux binary")
				t.Logf("found linux binary URL %s", b.URL)
			case "Windows":
				foundWindows = true
				assert.Equal(t, "https://github.com/roboll/helmfile/releases/download/v"+plugins.HelmfileVersion+"/helmfile_"+plugins.HelmfileVersion+"_Windows_amd64.tar.gz", b.URL, "URL for windows binary")
				t.Logf("found windows binary URL %s", b.URL)
			}
		}
	}
